...
public class R9TopomapTaker implements KeyListener{
	...
	public int[] startScan, saveButton, pauseButton, biasBox, currentBox;
...
//		System.out.println(NanonisNumber.parseNum("500m"));
		...
		int currentOption;
		if (nlayers > 1) currentOption = JOptionPane.showConfirmDialog(null, "Do you want to use a different setpoint for each layer?", "vary the current?", JOptionPane.YES_NO_OPTION);
	...
	private void getCoordinatesOfPoints() throws InterruptedException
	{
	...
	public void guideUser() throws InterruptedException
	{
		switch(stage)
		{
		case 0:
			JOptionPane.showMessageDialog(null, "(After closing this box) Move the mouse over the Start Scan button and press Spacebar");
			break;
		case 1:
			JOptionPane.showMessageDialog(null, "(After closing this box) Move the mouse over the Save Scan button and press Spacebar");
			break;
		case 2:
			JOptionPane.showMessageDialog(null, "(After closing this box) Move the mouse over the Pause After Scan button and press Spacebar");
			break;
		case 3:
			JOptionPane.showMessageDialog(null, "(After closing this box) Center the mouse over the numbers of the Bias Value and press Spacebar");
			break;
		case 4:
			if (current != null) 
				JOptionPane.showMessageDialog(null, "(After closing this box) Center the mouse over the numbers of the Setpoint Value and press Spacebar");
			else
				takeTime();
			break;
		case 5:
...
//			break;
	...
	private void takeTime() throws InterruptedException
	{
		try{
			...
			t = JOptionPane.showInputDialog(null, "Enter, in seconds, the amount of ADDITIONAL time you want to wait after each scan, to allow the tip to move back to the start and settle.");
			extraTime = (int)Double.parseDouble(t) +60;
			...
			t = JOptionPane.showInputDialog(null, "The map will begin shortly.\r\n" +
	...
	private void takeTheMap() throws InterruptedException
	{
		r = new Robo();
		...
		for (int i = 0; i < bias.length; i++)
		{
			...
			takeOneLayer(i);
			Thread.sleep((layerTime+extraTime)*1000);
		}
	...
	private void takeOneLayer(int i) throws InterruptedException
	{
		r.goTo(biasBox);
		Thread.sleep(50);
		r.doubleclick();
...
//			r.typeChar('a', true);
	...
	{
		R9TopomapTaker nt = new R9TopomapTaker();
	...
	public void keyTyped(KeyEvent arg0) {
		...
		{
		case 0:
			startScan = new int [] {x, y};
			break;
		case 1:
			saveButton = new int [] {x, y};
			break;
		case 2:
			pauseButton = new int [] {x, y};
			break;
		case 3:
			biasBox = new int [] {x, y};
			break;
